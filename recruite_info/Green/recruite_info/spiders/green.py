...
class GreenSpider(CrawlSpider):
    ...
    start_urls = ['https://www.green-japan.com/search/01?page={0}'.format(str(i)) for i in range(50,319)]
    ...
    def parse_item(self, response):
        ...
        res = response.css('ul.job-offer-meta-tags>li')
        for r in res:
            icon_name = r.css("span::attr('class')").get()
            if icon_name == "icon-salary":
                salary = r.css("li::text").get().strip()
                min_max_salary = re.split('[~〜]', salary)
                green["min_salary"] = min_max_salary[0]
                green["max_salary"] = min_max_salary[1]
            elif icon_name == "icon-site":
                green["locate"] = r.css("li::text").get().strip()
            elif icon_name == "icon-code":
                green["language"] = r.css("li::text").get().strip()
